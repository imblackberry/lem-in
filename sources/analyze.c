...
int		search_and_add_way(t_farm *farm, int *room_way_now, int way_length_now)
{
	...
	{
		new_way_ = new_way(ft_intdup(room_way_now, way_length_now),
										way_length_now);
	...
	{
		if (farm->map[last_id][i] == 1 && ft_intposition(room_way_now, i,
				way_length_now) == -1)
		{
...
int		search_top(t_farm *farm, t_way *way_now, int ants_come, int cur_steps)
{
	...
	while (way_now != NULL)
	{
		if (no_last_way_intersec(farm->cur_top, farm->cur_n_ids, way_now) == 1)
		{
			...
			farm->cur_n_ids++;
			ants_come_now = ants_come + change_ants_come(farm->cur_top,
														farm->cur_n_ids);
			cur_steps = way_now->length - 1 + count_steps(ants_come_now,
												farm->ants, farm->cur_n_ids);
			if (farm->top->steps == 0 || cur_steps < farm->top->steps)
				update_top(farm->top, farm->cur_top, farm->cur_n_ids,
							cur_steps);
